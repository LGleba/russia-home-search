...
def index(request):
    objs = []
    if (request.POST):
        parser = Parser(buy=request.POST['buy'], price_max=request.POST['price_max'], price_min=request.POST['price_min'], metro=request.POST['metro'], city=request.POST['city'], house_type=request.POST['house_type'])
    ...
    return render(request, 'blocks.html', context=data)
